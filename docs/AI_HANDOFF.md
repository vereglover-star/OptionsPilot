...
| GET | `/api/chain` | Option chain for a symbol/expiration — manual trading ticket data. Rows carry `strike/right/bid/ask/mid/delta/iv/gamma/theta/vega/volume/open_interest/liquidity/dte` plus, since **M4-C5**, `entry` (the expected fill), `breakeven` (priced at that fill, from `services/chain.py` — the same function the review modal uses), `chance_itm` (|delta| as a percentage, `null` when no greek was measured) and `greeks_derived` (whether `enrich_greeks` computed them, which closes `PRODUCT_STANDARDS.md` D3). `expiries` carries the server's DTE labels — **never recompute days to expiry in JavaScript** (§3.1) |
...
| GET | `/api/workspace` | Where the user was: tab, symbol, timeframe, indicators, extended hours, auto-follow, watchlist sort, ticket chart, recent symbols, saved layouts (V0.7.0), plus the selected `expiry` and `contract` and the `surface_level` (V0.11.0). Persisted in `settings.json` under a `workspace` key — **not** localStorage, which is a cache a cleared profile silently discards, and which a second client cannot see at all. `surface_level` is the one field **served here but stored elsewhere**, under its own top-level key: it is device-only where the workspace is not, and a client needs every context fact in one call to render a first frame |
| POST | `/api/workspace` | Merge a **partial** patch and return the full document. Partial by design: a client that only knows about `symbol` must be able to say so without overwriting panel layout it has never heard of. Unusable values fall back to their default rather than 4xx-ing, because this records where someone was looking — including `surface_level`, whose store-level setter is strict for every other caller. A patch that changes `symbol` drops the selected `contract`: a contract belongs to an underlying |
| DELETE | `/api/workspace` | Reset to the shipped defaults, saved layouts included. `surface_level` is **not** reset — it is stored outside the document and is closer to an accessibility setting than to a panel arrangement |
...
| GET | `/api/v1/quickpicks` | **M4-C1.** The four §6.3 intents as a catalogue (`key`, `label`, `right`, `target_dte`, `description`). The client renders the chips FROM this rather than restating them — the two-catalogue drift `services/guide.py` has a bidirectional test for |
| GET | `/api/v1/quickpick?symbol=&intent=&expiration=&right=` | **M4-C1.** Resolves one intent to a concrete contract. Two-phase inside: the expiry is chosen from dates alone, then that expiry's chain is fetched. Never fails silently — an unresolved pick carries a `reason` a user could read, and `strike` is `null` rather than `0.0`. A resolved one carries an `explanation` naming BOTH axes it chose (M4-C6), because an intent resolves a strike and an expiry and a user who disagrees needs to know which half to argue with |
| POST | `/api/v1/review` | **M4-C2.** §6.5's consequence restatement for an order draft: sentence, cost, maximum loss, breakeven beside spot, position size, "if you do nothing", plus the fill honesty line. **Prices with the broker's own model** (ask × (1 + slippage) to buy, bid × (1 − slippage) to sell), not the mid. Places nothing and mutates nothing, so it deliberately does not go through the idempotency path. An element that does not apply is `null` **with a note saying why**. M4-C4 added `buying_power{,_pct,_after,_note}` — affordability, a different question from position size and measured against cash rather than equity — and M4-C7 added `guided_note`, the ONE most consequential term, rendered at Guided Surface Level only |
...
# Tests (2849 tests as of this writing, all passing)
...
5. Frontend coverage is real but shallow — `tests/test_ui_server.py`
   exercises the FastAPI layer via `TestClient` (2849 tests cover this
