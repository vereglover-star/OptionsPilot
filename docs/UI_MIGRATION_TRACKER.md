...
| --- | --- | --- |
| 2026-08-07 | M4-C3…C11 | **M4 complete: the Trade destination.** One screen — chart above chain, an always-present ticket sharing Home's seam — reversing all three faults §6.1 names. **Six defects, none of them reported, every one found by building rather than by clicking.** (1) The ticket stated the MID as a cost: `tkSel.mid * qty * 100`, against a broker that crosses to the ask and applies slippage — measured on a live SPY chain, mid 40.40 against an expected fill of 40.804, so ten contracts read $404 light on the screen where a user decides whether they can afford it. (2) The sizing advisory divided by BUYING POWER and compared the result against a budget `RiskManager` measures on EQUITY, so it tripped at the wrong point on any account holding a position. (3) `Enter` in the chain both armed the ticket and opened the review, because the document-level shortcut tests `tkSel` and `selectContract` had just set it — the modal then stole focus out of the chain; `Enter` on an already-armed row now bubbles deliberately, which is what makes §6.7's `down down enter enter` work. (4) `#trade-chain` is a column flex container, so a realistic-length chain squashed the expiry strip to **57% of its natural height** — every pill present, readable by `textContent`, clickable, and unreadable; found only by growing the fixture from 3 strikes to 37, because a chain shorter than the region showing it cannot test anchoring. (5) A quick pick naming a strike the loaded chain did not hold returned silently. (6) The stop and trail fields had NO input handler, so the review described a trigger typed after the figures were last fetched, and `tkDraft` sent `stop: 0` for a trailing stop, dropping its clause from the review's first line. **Selection no longer re-renders the chain** — it used to rebuild ~300 rows of `innerHTML` and destroy the focused element doing it, so keyboard navigation could never have worked whatever was built on top. **PRODUCT_STANDARDS D3 closed:** rows now say whether their greeks were supplied or computed, a fact `chain_payload` already knew and discarded. **Gate 18 → 177**, one section per commit, with `[preserved]`/`[guard]`/`[deferred]` labels on the assertions that cannot fail against the previous build — and four times a reverted build made the gate CRASH rather than report, so every probe is now null-safe and every section bails with a failure when its precondition is missing. Token ratchets fell to 49 legacy type uses and 309 off-scale values. **Deviations carried forward:** draggable splitters (C3 — need a server-owned workspace field) and the legacy navigation (C11 — still the shell's rollback path; cleanest as M9-C7's existing scope, which also removes the flag's off-branch). `#tab-charts` was NOT deleted: it is the one chart instance's home and M6 owns it. |
| 2026-08-07 | M3.5-C1…C10 | **Home polish, run before M4 so the other five destinations inherit a corrected design language rather than a plausible one.** No backend touched: `/api/v1/home`, every service and every contract are byte-identical to M3. **Three reported problems were defects with an exact cause.** The rail's icons were sliced in half by a UA `<ul>` padding never reset — invisible at 200px, fatal at 72 and 56 where `overflow:hidden` clips; the primary navigation exposed **seven nameless links** below 1440px (a `display:none` label contributes nothing to the accessible name, so the keyboard hint became the name and then nothing did), specified in the design system and never built; and Home's three bands were touching at **0px** because `body.shell-v2 #home { display:block }` outranked `.home`'s `display:flex` and `gap` is inert on a block box. That last fix is a mechanism, not a number — **a visibility rule may only ever hide** — so M4–M6 cannot inherit it. Plus one seam: band 2 split 1.45fr:1fr and band 3 2fr:1fr, so the vertical seam stepped sideways 124px mid-page; now one `.dest-split` over `--split-major`/`--split-minor`. **Hierarchy:** metrics reordered by decision value (account equity had led at display size and is the one figure nobody acts on), the defined-but-unused 28px tier fills the 36→22 hole, and the instrument gained **three tiers** because one component used five times guarantees uniformity, which is the absence of hierarchy. What-to-do-next is the single focal region and is finally numbered — an `<ol>` whose markers `list-style:none` suppressed for two milestones while the component's comment called the ranking "the information". **Three more defects found while measuring, none reported:** positions grew without limit (14 open positions put band 3 at 1256px in a 1080px viewport; every payload the suite had ever driven held two); the equity chart's `viewBox` was 180 against a drawing height of 200, so the bottom of the curve was CLIPPED — the worst day on the chart was the one you could not see; and the LEGACY nav's responsive block used bare `nav` selectors, so below 1180px it was overriding the V2 shell — M2-C9 fixed that exact bug for the same block's base rules and never came down to the media query. Rail is two states now (216/64 at 1280), and the frame's wordmark box is exactly rail-width so one unbroken vertical line runs from the top of the window to the strip; `DESIGN_SYSTEM_V2.md` §5.2 updated rather than left to disagree. **Gates 34→60 shell, 33→50 Home**, plus two static rules: an unbalanced-CSS-comment check (added after a stray `*/` silently deleted `.hp-row`'s grid while every browser suite stayed green — it then caught the identical mistake in the next commit) and a viewBox/draw-height check. **Deviation carried forward:** the metric cluster still wraps at 1280/1024 rather than §2.12's `[More]` overflow. |
