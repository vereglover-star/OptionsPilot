...
| C2 | ✅ | Extract `ChartService` — smallest first, establishes the pattern | L | `74de6b2` |
...
| C7 | ✅ | Raise `ServiceError` from every service (H-7 applied) | L | `99998fb` |
| C8 | ✅ | Map `ServiceError` at the transport boundary | M | `7227223` |
| C9 | ✅ | Per-key idempotency locking with request fingerprints (N-1, N-2) | M | pending |
